...
{
  const int nx = chf.numCellsX;
  const int ny = chf.numCellsY;
  ...
  for (int y = 0; y < ny; ++y) {
    for (int x = 0; x < nx; ++x) {
      const rcCompactCell &c = chf.cells[x + y * nx];
      for (int i = (int)c.index, ni = (int)(c.index + c.count); i < ni; ++i) {
        ...
        for (int dir = 0; dir < 4; ++dir) {
          if (rcGetCon(s, dir) != RC_NOT_CONNECTED) {
            ...
            const int ai =
                (int)chf.cells[ax + ay * nx].index + rcGetCon(s, dir);
    ...
    for (int x = 0; x < nx; ++x) {
      ...
      for (int i = (int)c.index, ni = (int)(c.index + c.count); i < ni; ++i) {
        ...
        if (rcGetCon(s, 0) != RC_NOT_CONNECTED) {
          ...
          const int ai = (int)chf.cells[ax + ay * nx].index + rcGetCon(s, 0);
          ...
          if (rcGetCon(as, 3) != RC_NOT_CONNECTED) {
            ...
            const int aai =
                (int)chf.cells[aax + aay * nx].index + rcGetCon(as, 3);
        ...
        if (rcGetCon(s, 3) != RC_NOT_CONNECTED) {
          ...
          const int ai = (int)chf.cells[ax + ay * nx].index + rcGetCon(s, 3);
          const rcCompactSpan &as = chf.spans[ai];
          if (src[ai] + 2 < src[i]) src[i] = src[ai] + 2;
          ...
          if (rcGetCon(as, 2) != RC_NOT_CONNECTED) {
            ...
            const int aai =
                (int)chf.cells[aax + aay * nx].index + rcGetCon(as, 2);
            if (src[aai] + 3 < src[i]) src[i] = src[aai] + 3;
          }
        }
      }
    ...
    for (int x = nx - 1; x >= 0; --x) {
      ...
      for (int i = (int)c.index, ni = (int)(c.index + c.count); i < ni; ++i) {
        ...
        if (rcGetCon(s, 2) != RC_NOT_CONNECTED) {
          ...
          const int ai = (int)chf.cells[ax + ay * nx].index + rcGetCon(s, 2);
          const rcCompactSpan &as = chf.spans[ai];
          if (src[ai] + 2 < src[i]) src[i] = src[ai] + 2;
          ...
          if (rcGetCon(as, 1) != RC_NOT_CONNECTED) {
            ...
            const int aai =
                (int)chf.cells[aax + aay * nx].index + rcGetCon(as, 1);
            if (src[aai] + 3 < src[i]) src[i] = src[aai] + 3;
          }
        }
        if (rcGetCon(s, 1) != RC_NOT_CONNECTED) {
          ...
          const int ai = (int)chf.cells[ax + ay * nx].index + rcGetCon(s, 1);
          const rcCompactSpan &as = chf.spans[ai];
          if (src[ai] + 2 < src[i]) src[i] = src[ai] + 2;
          ...
          if (rcGetCon(as, 0) != RC_NOT_CONNECTED) {
            ...
            const int aai =
                (int)chf.cells[aax + aay * nx].index + rcGetCon(as, 0);
            if (src[aai] + 3 < src[i]) src[i] = src[aai] + 3;
          }
        }
      }
...
static void boxBlur(rcCompactHeightfield &chf,
                    int thr,
                    unsigned short *src,
                    unsigned short *dst)
{
        ...
        for (int dir = 0; dir < 4; ++dir) {
          if (rcGetCon(s, dir) != RC_NOT_CONNECTED) {
            ...
            if (rcGetCon(as, dir2) != RC_NOT_CONNECTED) {
              ...
              const int ai2 =
                  (int)chf.cells[ax2 + ay2 * nx].index + rcGetCon(as, dir2);
              d += (int)src[ai2];
            } else {
              d += cd;
            }
  ...
  while (stack.size() > 0) {
    ...
    for (int dir = 0; dir < 4; ++dir) {
      ...
      if (rcGetCon(cs, dir) != RC_NOT_CONNECTED) {
        ...
        const int ai = (int)chf.cells[ax + ay * nx].index + rcGetCon(cs, dir);
  ...
  while (stack.size() > 0) {
    ...
    for (int j = 0; j < stack.size(); j++) {
      ...
      for (int dir = 0; dir < 4; ++dir) {
        ...
        const int ax = x + rcGetDirOffsetX(dir);
        const int ay = y + rcGetDirOffsetY(dir);
        const int ai = (int)chf.cells[ax + ay * nx].index + rcGetCon(s, dir);
        if (chf.areas[ai] != area) continue;
                                       ...
                                       // as a bit shift
{
  ...
  for (unsigned int j = 0; j < nbStacks; ++j) stacks[j].clear();
                        ...
                        rcIntArray &cont)
{
  ...
  if (rcGetCon(ss, dir) != RC_NOT_CONNECTED) {
    const int ax = x + rcGetDirOffsetX(dir);
    const int ay = y + rcGetDirOffsetY(dir);
    const int ai = (int)chf.cells[ax + ay * xSize].index + rcGetCon(ss, dir);
    curReg = srcReg[ai];
  }
  cont.push(curReg);
  ...
  while (++iter < 40000) {
    ...
    if (isSolidEdge(chf, srcReg, x, y, i, dir)) {
      ...
      if (rcGetCon(s, dir) != RC_NOT_CONNECTED) {
        ...
        const int ay = y + rcGetDirOffsetY(dir);
        const int ai =
            (int)chf.cells[ax + ay * xSize].index + rcGetCon(s, dir);
        r = srcReg[ai];
      }
      if (r != curReg) {
        curReg = r;
      ...
      if (rcGetCon(s, dir) != RC_NOT_CONNECTED) {
        const rcCompactCell &nc = chf.cells[nx + ny * xSize];
...
{
  const int xSize = chf.numCellsX;
  const int ySize = chf.numCellsY;

  const int nreg = maxRegionId + 1;
  rcTempVector<rcRegion> regions;
  if (!regions.reserve(nreg)) {
    ctx.log(RC_LOG_ERROR,
            "mergeAndFilterRegions: Out of memory 'regions' (%d).", nreg);
    return false;
  }
  ...
  for (int y = 0; y < ySize; ++y) {
    for (int x = 0; x < xSize; ++x) {
      const rcCompactCell &c = chf.cells[x + y * xSize];
      for (int i = (int)c.index, ni = (int)(c.index + c.count); i < ni; ++i) {
        unsigned short r = srcReg[i];
    ...
    for (int i = 0; i < nreg; ++i) {
      ...
      for (int j = 0; j < reg.connections.size(); ++j) {
        ...
        if (mreg.id == 0 || (mreg.id & RC_BORDER_REG) || mreg.overlap)
          continue;
        if (mreg.spanCount < smallest && canMergeWithRegion(reg, mreg) &&
                                       ...
                                       unsigned short *srcReg)
{
  ...
  if (!regions.reserve(nreg)) {
    ctx.log(RC_LOG_ERROR,
            "mergeAndFilterLayerRegions: Out of memory 'regions' (%d).", nreg);
    return false;
  }
  for (int i = 0; i < nreg; ++i)
    regions.push_back(rcRegion((unsigned short)i));
  ...
  for (int i = 1; i < nreg; ++i) {
    ...
    while (stack.size() > 0) {
      ...
      for (int j = 0; j < ncons; ++j) {
        ...
        // Skip if different area type, do not connect regions with different
        // area type.
...
bool rcBuildDistanceField(rcContext &ctx, rcCompactHeightfield &chf)
{
  ...
  // FIXME get rid of rcAlloc in this function

  unsigned short *src = (unsigned short *)rcAlloc(
      sizeof(unsigned short) * chf.spanCount, RC_ALLOC_TEMP);
  if (!src) {
    ctx.log(RC_LOG_ERROR, "rcBuildDistanceField: Out of memory 'src' (%d).",
            chf.spanCount);
    return false;
  }
  unsigned short *dst = (unsigned short *)ctx.tmpAlloc(
      sizeof(unsigned short) * chf.spanCount);
  if (!dst) {
    ctx.log(RC_LOG_ERROR, "rcBuildDistanceField: Out of memory 'dst' (%d).",
            chf.spanCount);
    rcFree(src);
    return false;
  }
...
/// mergeRegionArea helps reduce unnecessarily small regions.
///
                            ...
                            const int mergeRegionArea)
{
  ...
  rcScopedDelete<unsigned short> srcReg((unsigned short *)rcAlloc(
      sizeof(unsigned short) * chf.spanCount, RC_ALLOC_TEMP));
  if (!srcReg) {
    ctx.log(RC_LOG_ERROR, "rcBuildRegionsMonotone: Out of memory 'src' (%d).",
            chf.spanCount);
    return false;
  }
  memset(srcReg, 0, sizeof(unsigned short) * chf.spanCount);

  const int nsweeps = rcMax(xSize, ySize);
  rcScopedDelete<rcSweepSpan> sweeps(
      (rcSweepSpan *)rcAlloc(sizeof(rcSweepSpan) * nsweeps, RC_ALLOC_TEMP));
  if (!sweeps) {
    ctx.log(RC_LOG_ERROR,
            "rcBuildRegionsMonotone: Out of memory 'sweeps' (%d).", nsweeps);
    return false;
  }
  ...
  if (borderSize > 0) {
    ...
    const int bw = rcMin(xSize, borderSize);
    const int bh = rcMin(ySize, borderSize);
    // Paint regions
    paintRectRegion(0, bw, 0, ySize, id | RC_BORDER_REG, chf, srcReg);
    id++;
    paintRectRegion(xSize - bw, xSize, 0, ySize, id | RC_BORDER_REG, chf,
                    srcReg);
    id++;
    paintRectRegion(0, xSize, 0, bh, id | RC_BORDER_REG, chf, srcReg);
    id++;
    paintRectRegion(0, xSize, ySize - bh, ySize, id | RC_BORDER_REG, chf,
                    srcReg);
    id++;
  }
    ...
    for (int x = borderSize; x < xSize - borderSize; ++x) {
      ...
      for (int i = (int)c.index, ni = (int)(c.index + c.count); i < ni; ++i) {
        ...
        if (rcGetCon(s, 0) != RC_NOT_CONNECTED) {
          ...
          const int ay = y + rcGetDirOffsetY(0);
          const int ai =
              (int)chf.cells[ax + ay * xSize].index + rcGetCon(s, 0);
        ...
        if (rcGetCon(s, 3) != RC_NOT_CONNECTED) {
          ...
          const int ay = y + rcGetDirOffsetY(3);
          const int ai =
              (int)chf.cells[ax + ay * xSize].index + rcGetCon(s, 3);
...

  {
    ...
    rcIntArray overlaps;
    chf.maxRegions = id;
    if (!mergeAndFilterRegions(ctx, minRegionArea, mergeRegionArea,
                               chf.maxRegions, chf, srcReg, overlaps))
      return false;
                    ...
                    const int mergeRegionArea)
{
  ...
  if (!buf) {
    ctx.log(RC_LOG_ERROR, "rcBuildRegions: Out of memory 'tmp' (%d).",
            chf.spanCount * 4);
    return false;
  }
  ...
  for (int i = 0; i < NB_STACKS; ++i) lvlStacks[i].reserve(256);
  ...
  if (borderSize > 0) {
    ...
    // Paint regions
    paintRectRegion(0, bw, 0, ySize, regionId | RC_BORDER_REG, chf, srcReg);
    regionId++;
    paintRectRegion(xSize - bw, xSize, 0, ySize, regionId | RC_BORDER_REG, chf,
                    srcReg);
    regionId++;
    paintRectRegion(0, xSize, 0, bh, regionId | RC_BORDER_REG, chf, srcReg);
    regionId++;
    paintRectRegion(0, xSize, ySize - bh, ySize, regionId | RC_BORDER_REG, chf,
                    srcReg);
    regionId++;
  }
  ...
  while (level > 0) {
    ...
    if (sId == 0)
      ...
      appendStacks(lvlStacks[sId - 1], lvlStacks[sId],
...

    {
      ...
      expandRegions(expandIters, level, chf, srcReg, srcDist, lvlStacks[sId],
                    false);
    }
...

  {
    ...
    if (overlaps.size() > 0) {
      ctx.log(RC_LOG_ERROR, "rcBuildRegions: %d overlapping regions.",
              overlaps.size());
    }
  }
                         ...
                         const int minRegionArea)
{
  ...
  rcScopedDelete<unsigned short> srcReg((unsigned short *)rcAlloc(
      sizeof(unsigned short) * chf.spanCount, RC_ALLOC_TEMP));
  if (!srcReg) {
    ctx.log(RC_LOG_ERROR, "rcBuildLayerRegions: Out of memory 'src' (%d).",
            chf.spanCount);
    return false;
  }
  memset(srcReg, 0, sizeof(unsigned short) * chf.spanCount);
  ...
  rcScopedDelete<rcSweepSpan> sweeps(
      (rcSweepSpan *)rcAlloc(sizeof(rcSweepSpan) * nsweeps, RC_ALLOC_TEMP));
  if (!sweeps) {
    ctx.log(RC_LOG_ERROR, "rcBuildLayerRegions: Out of memory 'sweeps' (%d).",
            nsweeps);
    return false;
  }
...

  {
    ...
    chf.maxRegions = id;
    if (!mergeAndFilterLayerRegions(ctx, minRegionArea, chf.maxRegions, chf,
                                    srcReg))
      return false;
  }
