                                ...
                                const rcHeightPatch &hp)
{
  ...
  if (h == RC_UNSET_HEIGHT) {
    ...
    for (int i = 0; i < maxIter; i++) {
      ...
      // possible. This means that if we find a height in one of the neighbor
      // cells to the center, we don't want to expand further out than the 8
      // neighbors - we want to limit our search to the closest of these
      // "rings", but the best height in the ring. For example, the center is
      // just 1 cell. We checked that at the entrance to the function. The next
      // "ring" contains 8 cells (marked 1 above). Those are all the neighbors
      // to the center cell. The next one again contains 16 cells (marked 2).
      // In general each ring has 8 additional cells, which can be thought of
      // as adding 2 cells around the "center" of each side when we expand the
      // ring. Here we detect if we are about to enter the next ring, and if we
...
{
  if (nedges >= maxEdges) {
    ctx->log(RC_LOG_ERROR, "addEdge: Too many edges (%d/%d).", nedges,
             maxEdges);
    return EV_UNDEF;
  }
                         ...
                         int t1)
{
  for (int i = 0; i < nedges; ++i) {
    ...
    if (overlapSegSeg2d(&pts[s0 * 3], &pts[t0 * 3], &pts[s1 * 3],
                        &pts[t1 * 3]))
      return true;
  }
  return false;
  ...
  for (int u = 0; u < npts; ++u) {
    ...
    if (vcross2(&pts[s * 3], &pts[t * 3], &pts[u * 3]) > EPS) {
      ...
      if (d > r * (1 + tol)) {
        ...
        // Inside epsilon circum circle, do extra tests to make sure the edge
        // is valid. s-u and t-u cannot overlap with s-pt nor t-pt if they
        // exists.
                         ...
                         rcIntArray &edges)
{
  ...
  for (int i = 0, j = nhull - 1; i < nhull; j = i++)
    addEdge(ctx, &edges[0], nedges, maxEdges, hull[j], hull[i], EV_HULL,
            EV_UNDEF);

  int currentEdge = 0;
  while (currentEdge < nedges) {
    if (edges[currentEdge * 4 + 2] == EV_UNDEF)
      completeFacet(ctx, pts, npts, &edges[0], nedges, maxEdges, nfaces,
                    currentEdge);
  ...
  for (int i = 0; i < tris.size() / 4; ++i) {
    ...
    if (t[0] == -1 || t[1] == -1 || t[2] == -1) {
      ctx->log(RC_LOG_WARNING,
               "delaunayHull: Removing dangling face %d [%d,%d,%d].", i, t[0],
               t[1], t[2]);
  ...
  for (int i = 0; i < nhull; i++) {
    if (hull[i] >= nin)
      continue;  // Ears are triangles with original vertices as middle vertex
                 // while others are actually line segments on edges
    int pi = prev(i, nhull);
    int ni = next(i, nhull);
...
{
  // All internal sampled points come after the hull so we can early out for
  // those.
  ...
  if (sampleDist > 0) {
    for (int i = 0, j = nin - 1; i < nin; j = i++) {
      ...
      for (int k = 0; k <= nn; ++k) {
        ...
        pos[1] = vj[1] + dy * u;
        pos[2] = vj[2] + dz * u;
        pos[1] = getHeight(pos[0], pos[1], pos[2], cs, ics, chf.ch,
  ...
  if (sampleDist > 0) {
    ...
    for (int z = z0; z < z1; ++z) {
      for (int x = x0; x < x1; ++x) {
        ...
        samples.push(x);
        samples.push(getHeight(pt[0], pt[1], pt[2], cs, ics, chf.ch,
                               heightSearchRadius, hp));
        samples.push(z);
    ...
    for (int iter = 0; iter < nsamples; ++iter) {
      ...
      for (int i = 0; i < nsamples; ++i) {
        ...
        // triangulations which are cause by symmetrical data from the grid
        // structure.
                                    ...
                                    rcIntArray &array)
{
  ...
  while (true) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      array.push(newX);
      array.push(newY);
      array.push((int)chf.cells[(newX + bs) + (newY + bs) * chf.width].index +
                 rcGetCon(cs, dir));
    }
  ...
  while (head * 3 < queue.size()) {
    ...
    if (head >= RETRACT_SIZE) {
      ...
      if (queue.size() > RETRACT_SIZE * 3)
        memmove(&queue[0], &queue[RETRACT_SIZE * 3],
                           ...
                           rcPolyMeshDetail &dmesh)
{
  ...
  if (!poly) {
    ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetail: Out of memory 'poly' (%d).",
             nvp * 3);
    return false;
  }
  ...
  for (int i = 0; i < mesh.npolys; ++i) {
    ...
    getHeightData(ctx, chf, p, npoly, mesh.verts, borderSize, hp, arr,
                  mesh.regs[i]);

    // Build detail mesh.
    int nverts = 0;
    if (!buildPolyDetail(ctx, poly, npoly, sampleDist, sampleMaxError,
                         heightSearchRadius, chf, hp, verts, nverts, tris,
                         edges, samples)) {
      return false;
    }
