                           ...
                           bool &isBorderVertex)
{
  ...
  if (rcGetCon(s, dir) != RC_NOT_CONNECTED) {
    ...
    const int ai =
        (int)chf.cells[ax + ay * chf.numCellsX].index + rcGetCon(s, dir);
    ...
    if (rcGetCon(as, dirp) != RC_NOT_CONNECTED) {
      ...
      const int ai2 =
          (int)chf.cells[ax2 + ay2 * chf.numCellsX].index + rcGetCon(as, dirp);
  ...
  if (rcGetCon(s, dirp) != RC_NOT_CONNECTED) {
    ...
    const int ai =
        (int)chf.cells[ax + ay * chf.numCellsX].index + rcGetCon(s, dirp);
    const rcCompactSpan &as = chf.spans[ai];
    ch = rcMax(ch, (int)as.z);
    ...
    if (rcGetCon(as, dir) != RC_NOT_CONNECTED) {
      ...
      const int ai2 =
          (int)chf.cells[ax2 + ay2 * chf.numCellsX].index + rcGetCon(as, dir);
      const rcCompactSpan &as2 = chf.spans[ai2];
      ch = rcMax(ch, (int)as2.z);
      regs[2] = chf.spans[ai2].reg | (chf.areas[ai2] << 16);
    }
  ...
  // Check if the vertex is special edge vertex, these vertices will be removed
  // later.
  for (int j = 0; j < 4; ++j) {
    const int a = j;
    const int b = (j + 1) & 0x3;
    ...
    // row, followed by two interior cells and none of the regions are out of
    // bounds.
                        ...
                        rcIntArray &points)
{
  ...
  while (++iter < 40000) {
    if (flags[i] & (1 << dir)) {
      ...
      const rcCompactSpan &s = chf.spans[i];
      if (rcGetCon(s, dir) != RC_NOT_CONNECTED) {
        const rcCompactCell &nc = chf.cells[nx + ny * chf.numCellsX];
                            ...
                            const int buildFlags)
{
  ...
  if (hasConnections) {
    ...
    for (int i = 0, ni = points.size() / 4; i < ni; ++i) {
      int ii = (i + 1) % ni;
      const bool differentRegs = (points[i * 4 + 3] & RC_CONTOUR_REG_MASK) !=
  ...
  for (int i = 0; i < simplified.size() / 4;) {
    ...
    if ((points[ci * 4 + 3] & RC_CONTOUR_REG_MASK) == 0 ||
      ...
      while (ci != endi) {
        float d = distancePtSeg(points[ci * 4 + 0], points[ci * 4 + 1], ax, ay,
                                bx, by);
        if (d > maxd) {
          maxd = d;
          maxi = ci;
...
template <size_t size, auto cmp_fn>
void bad_sort(void *base, size_t nitems) {

  if (nitems < 2) return;

  // Stack to store left and right indices for subarrays
  size_t stack[32];
  int top = -1;
...
void sortHoles(rcContourHole *holes, size_t num_holes)
{
  bad_sort<sizeof(rcContourHole), compareHoles>(holes, num_holes);
}

void sortDiags(rcPotentialDiagonal *diags, size_t ndiags)
{
  bad_sort<sizeof(rcPotentialDiagonal), compareDiagDist>(diags, ndiags);
}
...
void sortHoles(rcContourHole *holes, size_t num_holes)
{
  qsort(holes, num_holes, sizeof(rcContourHole), compareHoles);
}
...
static void mergeRegionHoles(rcContext &ctx, rcContourRegion &region)
{
  ...
  for (int i = 0; i < region.nholes; i++)
    findLeftMostVertex(region.holes[i].contour, &region.holes[i].minx,
                       &region.holes[i].miny, &region.holes[i].leftmost);
  ...
  if (!diags) {
    ctx.log(RC_LOG_WARNING, "mergeRegionHoles: Failed to allocated diags %d.",
             maxVerts);
    return;
  }
  ...
  for (int i = 0; i < region.nholes; i++) {
    ...
    for (int iter = 0; iter < hole->nverts; iter++) {
      ...
      // The 'best' vertex must be in the cone described by 3 consecutive
      ...
      // Sort potential diagonals by distance, we want to make the connection
      // as short as possible.
      sortDiags(diags, ndiags);

      // Find a diagonal that is not intersecting the outline not the remaining
      // holes.
      index = -1;
      for (int j = 0; j < ndiags; j++) {
        const int *pt = &outline->verts[diags[j].vert * 4];
        bool intersect = intersectSegContour(pt, corner, diags[i].vert,
                                             outline->nverts, outline->verts);
...
/// and @p maxEdgeLen parameters control how closely the simplified contours
/// will match the raw contours.
///
                     ...
                     const int buildFlags)
{
  ...
  cset.cs = chf.cs;
  cset.ch = chf.ch;
  cset.numCellsX = xSize - chf.borderSize * 2;
  cset.numCellsY = ySize - chf.borderSize * 2;
  ...
  if (!flags) {
    ctx.log(RC_LOG_ERROR, "rcBuildContours: Out of memory 'flags' (%d).",
             chf.spanCount);
    return false;
  }
    ...
    for (int x = 0; x < xSize; ++x) {
      ...
      for (int i = (int)c.index, ni = (int)(c.index + c.count); i < ni; ++i) {
        ...
        if (simplified.size() / 4 >= 3) {
          ...
          cont->verts =
              (int *)rcAlloc(sizeof(int) * cont->nverts * 4, RC_ALLOC_PERM);
          if (!cont->verts) {
          ...
          cont->rverts =
              (int *)rcAlloc(sizeof(int) * cont->nrverts * 4, RC_ALLOC_PERM);
          if (!cont->rverts) {
            ctx.log(RC_LOG_ERROR,
  ...
  if (cset.nconts > 0) {
    ...
    rcScopedDelete<signed char> winding((signed char *)rcAlloc(
        sizeof(signed char) * cset.nconts, RC_ALLOC_TEMP));
    if (!winding) {
      ctx.log(RC_LOG_ERROR, "rcBuildContours: Out of memory 'hole' (%d).",
               cset.nconts);
      return false;
    }
    int nholes = 0;
    ...
    if (nholes > 0) {
      ...
      if (!holes) {
        ctx.log(RC_LOG_ERROR, "rcBuildContours: Out of memory 'holes' (%d).",
                 cset.nconts);
        return false;
      }
      ...
      for (int i = 0; i < nregions; i++) {
        ...
        if (reg.outline) {
          ...
          ctx.log(RC_LOG_ERROR,
                   "rcBuildContours: Bad outline for region %d, contour "
                   "simplification is likely too aggressive.",
                   i);
        }
      }
    }
