...
/// @param[in]		hint	A hint to the allocator on how long the memory
...
inline void *rcAlloc(size_t size, rcAllocHint hint)
{
  (void)hint;
  return madrona::rawAlloc(size);
}
...
///
/// @param[in]		ptr		A pointer to a memory block previously
/// allocated using #rcAlloc.
///
/// @see rcAlloc, rcAllocSetCustom
inline void rcFree(void *ptr)
{
  madrona::rawDealloc(ptr);
}
...
/// Signed to avoid warnings when comparing to int loop indexes, and common
...
/// benchmark that demonstrates a performance improvement before introducing
/// use cases.
...
///  * Omits methods of limited utility: insert/erase, (bad performance), at
///  (we don't use exceptions), operator=.
...
class rcVectorBase {
  ...
  rcSizeType m_cap;
  T *m_data;
  // Constructs a T at the give address with either the copy constructor or the
  // default.
